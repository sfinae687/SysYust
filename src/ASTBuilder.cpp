...
//    auto thePath = R"(E:\Source\SysYust\compiler2023\TestCase\functional\61_sort_test7.sy)";
    ...
    auto outFilePath = programPath.stem().replace_extension(".out")  ;
    auto logFilePath = programPath.filename().replace_extension(".log");
...
//    std::ofstream log_file(logFilePath);
    ...
    LOG_INFO("Current path is {}", std::filesystem::current_path().string());
    LOG_INFO("outFile path is {}", outFilePath.string());
    ...
    // assert(0);
    int ret = interp.enter(astTree.get());
